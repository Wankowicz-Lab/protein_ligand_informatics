...
sns.violinplot(data=df, x='group', y='score', palette=palette)
plt.title('Top 3 P2Rank Scores (Full Data)')
plt.grid(True)
plt.savefig('/dors/wankowicz_lab/ellas/p2rank_2.5/p2rank_results/violin_full.png')
plt.close()

# === 2. 3σ Cutoff Violin Plot ===
cutoff_3sigma = df.groupby('group')['score'].transform(lambda x: x.mean() + 3 * x.std())
df_3sigma = df[df['score'] <= cutoff_3sigma]

plt.figure(figsize=(10, 6))
sns.violinplot(data=df_3sigma, x='group', y='score', palette=palette)
plt.title('Top 3 P2Rank Scores (≤ μ + 3σ)')
plt.grid(True)
plt.savefig('/dors/wankowicz_lab/ellas/p2rank_2.5/p2rank_results/violin_3sigma.png')
plt.close()

# === 3. 95th Percentile Cutoff Violin Plot ===
cutoff_95 = df.groupby('group')['score'].transform(lambda x: x.quantile(0.95))
df_95 = df[df['score'] <= cutoff_95]

plt.figure(figsize=(10, 6))
sns.violinplot(data=df_95, x='group', y='score', palette=palette)
plt.title('Top 3 P2Rank Scores (≤ 95th Percentile)')
plt.grid(True)
plt.savefig('/dors/wankowicz_lab/ellas/p2rank_2.5/p2rank_results/violin_95percentile.png')
plt.close()
